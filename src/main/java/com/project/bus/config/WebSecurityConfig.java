package com.project.bus.config;

import com.project.bus.service.UserService;
...
public class WebSecurityConfig  extends WebSecurityConfigurerAdapter { // Spring Security의 설정파일로서의 역할을 하기 위해 상속해야 하는 클래스
  ...
  protected void configure(HttpSecurity http) throws Exception { // http 관련 인증 설정
    http
            ...
            .antMatchers("/login", "/signup", "/user").permitAll() // permitAll 모두 접근 허용
            .antMatchers("/", "/bus/list", "/bus/reserve", "/bus/reserve/list", "/main", "/bus/view").hasRole("USER") // 로그인 한 사용자, 관리자 접근 가능
            .antMatchers("/admin", "/bus/write", "/bus/delete/**","/bus/modify/**", "/bus/update/{id}").hasRole("ADMIN") // 오직 관리자만 접근 가능
            .anyRequest().authenticated() // 나머지 요청들은 권한의 종류에 상관 없이 권한이 있어야 접근 가능
            .and()
            .formLogin() // 로그인에 관한 설정
            .loginPage("/login") // 로그인 페이지 링크
            .defaultSuccessUrl("/bushome") // 로그인 성공 후 리다이렉트 주소
            .and()
            .logout() // 로그아웃
